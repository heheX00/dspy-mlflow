...
class NLToQuerySignature(dspy.Signature):
    ...
    - For top-N or ranking requests, prefer size=0 with aggregations.
    ...
    - Do not invent fields not present in es_schema.
...
class NLToQueryDSL(dspy.Module):
    ...
    def forward(self, nl_query: str) -> dspy.Prediction:
        ...
        for _ in range(self.max_refine_attempts + 1):
            judge_result = self.dspy_judge.evaluate_query_dsl(generated_query_dsl=current_query_dsl)
