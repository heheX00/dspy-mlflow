...
class ESClient:
    """Unified Elasticsearch client for both normal and sandbox evaluation flows."""
    ...
    def __init__(
        ...
        resolved_host = host or getattr(settings, "sandbox_es_host", None) or settings.es_host
        resolved_username = (
            username
            if username is not None
            else getattr(settings, "sandbox_es_username", None) or settings.es_username
        )
        resolved_password = (
            password
            if password is not None
            else getattr(settings, "sandbox_es_password", None) or settings.es_password
        )
        resolved_index = index or getattr(settings, "sandbox_es_index", None) or settings.es_index
        resolved_verify_ssl = (
            verify_ssl
            if verify_ssl is not None
            else getattr(settings, "sandbox_es_verify_ssl", None)
        )
        if resolved_verify_ssl is None:
            resolved_verify_ssl = settings.es_verify_ssl

        self.es = Elasticsearch(
            hosts=[resolved_host],
            basic_auth=(resolved_username, resolved_password),
            verify_certs=resolved_verify_ssl,
        )
    ...
    def get_last_x_days_samples(self, days: int, date_field: str = "@timestamp") -> list[dict]:
        ...
        for days_ago in range(1, days + 1):
            ...
            query = {
                ...
                "query": {
                    "function_score": {
                        "query": {
                            "range": {date_field: {"gte": start_date, "lt": end_date}}
                        },
                        "random_score": {},
                    }
                },
        ...
        for day_result in results:
            body = day_result.body if hasattr(day_result, "body") else day_result
            hits = body.get("hits", {}).get("hits", [])
            all_sampled_docs.extend([hit.get("_source", {}) for hit in hits])
        return all_sampled_docs

    def flatten_es_mapping(self) -> dict[str, str]:
        ...
        def extract_properties(prop_dict: dict[str, Any], current_path: str = "") -> None:
            for field_name, field_info in prop_dict.items():
                ...
                if (
                    ...
                    and isinstance(field_info["fields"], dict)
                    and "keyword" in field_info["fields"]
                ):
                    flat_fields[f"{new_path}.keyword"] = "keyword"

                if "fields" in field_info and isinstance(field_info["fields"], dict):
                    for subfield_name, subfield_info in field_info["fields"].items():
                        subfield_type = subfield_info.get("type", "unknown")
                        flat_fields[f"{new_path}.{subfield_name}"] = subfield_type

        extract_properties(properties)
        return flat_fields

    def search(self, query_dsl: dict[str, Any]) -> dict[str, Any]:
        query = self.normalize_query_dsl(query_dsl)
    ...
    def validate_query_dsl(
        ...
        return self.evaluate_query_dsl(
            query_dsl=query_dsl,
    ...
    def evaluate_query_dsl(
        ...
        execution_score, execution_feedback, execution_details = self._score_execution(normalized)
    ...
    def _score_execution(self, query_dsl: dict[str, Any]) -> tuple[float, str, dict[str, Any]]:
    ...
    def _contains_key(node: Any, target_key: str) -> bool:
        if isinstance(node, dict):
            for key, value in node.items():
                if key == target_key:
                    return True
                if ESClient._contains_key(value, target_key):
                    return True
        elif isinstance(node, list):
            return any(ESClient._contains_key(item, target_key) for item in node)
        return False
        ...
        if isinstance(node, dict):
            for key, value in node.items():
                if key == target_key and isinstance(value, int):
                    values.append(value)
                values.extend(ESClient._find_numeric_values_for_key(value, target_key))
        elif isinstance(node, list):
            for item in node:
                values.extend(ESClient._find_numeric_values_for_key(item, target_key))
        return values
        ...
        if isinstance(node, dict):
            for key, value in node.items():
                if key == "terms" and isinstance(value, dict):
                    ...
                    if isinstance(size, int):
                        sizes.append(size)
                sizes.extend(ESClient._find_terms_agg_sizes(value))
        elif isinstance(node, list):
            for item in node:
                sizes.extend(ESClient._find_terms_agg_sizes(item))
        return sizes
